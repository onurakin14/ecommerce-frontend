...
function AdminProductDetail() {
    ...
    const changeMainImage = (imageUrl: string) => {
        let imageCard = document.getElementById("mainImageCard");
    ...
    return (
        <React.Fragment>
            {isEditModalActive && <ProductFormModal product={product} onClose={closeModals} />}
            {isDeleteModalActive && <DeleteConfirmModal product={product} onClose={closeModals} />}
                    ...
                    <div className="max-w-[1200px] mx-auto flex flex-col gap-6">
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                            <div className="flex flex-col gap-1">
                                <h1 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">{product?.title}</h1>
                                <p className="text-slate-500 text-sm">Manage product details, pricing and stock.</p>
                            </div>
                            <div className="flex items-center gap-3">
                                <button onClick={() => setDeleteModalActive(true)} className="flex items-center gap-2 px-4 py-2 rounded-lg border border-red-200 text-red-600 hover:bg-red-50 dark:border-red-900 dark:text-red-400 dark:hover:bg-red-900/20 font-medium text-sm transition-colors">
                                    <span className="material-symbols-outlined text-[20px]">delete</span>
                                    <span>Delete</span>
                                </button>
                                <button onClick={() => setEditModalActive(true)} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-primary text-white hover:bg-primary/90 shadow-md shadow-primary/20 font-medium text-sm transition-all">
                            ...
                            <div className="lg:col-span-5 flex flex-col gap-4">
                                ...
                                <div className="bg-white dark:bg-slate-900 rounded-xl p-2 shadow-sm border border-slate-100 dark:border-slate-800">
                                    <div className="aspect-square w-full rounded-lg bg-slate-50 dark:bg-slate-800 relative overflow-hidden group">
                                        <div id="mainImageCard" className="w-full h-full bg-center bg-contain bg-no-repeat transition-transform duration-500 group-hover:scale-105" data-alt="Titanium finish smartphone back view" style={{ backgroundImage: `url("${product?.thumbnail}")` }}></div>
                                ...
                                <div className="grid grid-cols-4 gap-4">
                                    {product?.images.map((item, index) => {
                                        return (
                                            <button key={index} onClick={() => changeMainImage(item)} className="aspect-square rounded-lg border-2 border-primary p-1 bg-white dark:bg-slate-900 cursor-pointer">
