    ...
    return (
        <React.Fragment>
            {Array.from({ length: fullStars }).map((_, i) => (
                <span key={`${id}-fullStar-${i}`} style={{ fontVariationSettings: "'FILL' 1" }}
            ...
            {hasHalfStar && (
                <span
                    key={`halfStar-${id}`} style={{ fontVariationSettings: "'FILL' 1" }}
                    className="material-symbols-outlined text-yellow-500 !text-base">
                    star_half
                </span>
            )}
            ...
            {Array.from({ length: maxStars - fullStars - (hasHalfStar ? 1 : 0) }).map((_, i) => (
                <span
                    key={`noStar-${id}-${i}`} style={{ fontVariationSettings: "'FILL' 1" }}
